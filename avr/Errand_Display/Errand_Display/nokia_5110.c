...
/* DC & CE Toggle Macros */
#define NOKIA_DC_HIGH PORTD |= (1<<DC) //PD5
#define NOKIA_DC_LOW PORTD &= ~(1<<DC) //PD5
#define NOKIA_CE_HIGH PORTB |= (1<<CE) //PB2
#define NOKIA_CE_LOW PORTB &= ~(1<<CE) //PB2
...
void nokia__cmd(uint8_t cmd) {
	 ...
	 NOKIA_CE_LOW;		//Selects 5110 by setting CE to LOW
	 spi__write(cmd);	//Sends a byte via SPI to the LCD
	 NOKIA_CE_HIGH;		//De-selects 5110 by setting CE to HIGH
}
...
void nokia__init() {
	...
	//DDRB = (1<<CE); //Sets CE as output
	NOKIA_RST_HIGH;
	NOKIA_CE_HIGH;				//Sets CE pin to HIGH so when cmd or data are utilized, CE goes from HIGH to LOW
	_delay_ms(10);
	NOKIA_RST_LOW;
	_delay_ms(70);
	NOKIA_RST_HIGH;
